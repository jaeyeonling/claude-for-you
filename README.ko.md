...
# <!-- CAP-DOCS: 50/128 값은 MAX_ALLOWED_MODELS_PER_KEY
#      (src/auth/api-key-store.ts)와 MAX_MODEL_PATTERN_LENGTH
#      (src/auth/model-allow.ts)와 동기화 유지. UI label은 상수에서
#      자동 갱신되지만 이 문서 스니펫은 수동 갱신 필요. -->
...
| `PATCH /admin/keys/:name` | JSON | 발급된 file 키 in-place 수정 — `{ newName?, allowedModels? }`. POST와 동일 상한. 폼 미러: `POST /admin/keys/:name/update`. env-baked 키는 `env_source_immutable`로 거부. rename: `-d '{"newName":"carol-restricted"}'`. |
