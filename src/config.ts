...
export type AppConfig = Readonly<{
  ...
  /** Per-key concurrent in-flight cap. 0 = disabled. Layered below the
   * global cap to prevent one (buggy or compromised) key from starving others. */
...
export const loadConfig = (env: NodeJS.ProcessEnv = process.env): AppConfig => {
  ...
  return Object.freeze({
    ...
    maxConcurrentRequestsPerKey: Number(env.MAX_CONCURRENT_REQUESTS_PER_KEY ?? 0),
