...
- **Cache control**: caller-emitted blocks carry `cache_control: { type: "ephemeral" }` for prompt caching (not shown above — strip when reusing captured blocks as synthesized values). Note: §2a below covers a *separate* proxy-emitted CC_BLOCK that also carries `cache_control: { type: 'ephemeral' }` — same syntax, different owner. (Exception: in the at-cap branch added by #136, the proxy prepends `CC_BLOCK_NO_CACHE`, which has NO `cache_control` — see §2a.) Do not confuse the two when reading wire captures.
...
2. **Prepend (cap-aware as of #136)** — when the caller's `system` is a string, an empty/missing value, or an array without a canonical marker, `ensureSystem` prepends a CC identity block as the first element. Tomodachi-style bots and SDK clients without the marker flow through this branch — the PR #41 entitlement fix is preserved. The prepended block's `cache_control` is conditional on how many breakpoints the caller already carries (`pickPrependBlock` + `countCacheControlBlocks` in `src/proxy/messages.ts`):
   - **Sub-cap caller** (fewer than 4 `cache_control` breakpoints across system+tools+messages): prepend `{ type: 'text', text: CC_SYSTEM_PREFIX, cache_control: { type: 'ephemeral' } }` (`CC_BLOCK`) — the anchor is preserved (issue #55).
   - **At-cap caller** (already at the 4-breakpoint ceiling — e.g. NousResearch/hermes-agent's hardcoded `system_and_3` layout: 1 system + 3 message-content breakpoints): prepend `{ type: 'text', text: CC_SYSTEM_PREFIX }` (`CC_BLOCK_NO_CACHE`, no `cache_control`). A 5th breakpoint would make Anthropic reject the whole request (`A maximum of 4 blocks with cache_control may be provided. Found 5`), so the proxy contributes identity text without consuming a breakpoint and leaves the caller's 4 intact. The caller's body is never mutated either way.

   The exact boundary, counting the caller's own breakpoints `n` *before* the proxy prepend (`pickPrependBlock` in `src/proxy/messages.ts`): `n + 1 <= 4` → `CC_BLOCK`; `n + 1 > 4` (i.e. `n >= 4`) → `CC_BLOCK_NO_CACHE`. In short — **`n ≤ 3` sub-cap → `CC_BLOCK` (lands at/under the ceiling); `n = 4` at-cap → `CC_BLOCK_NO_CACHE`; `n ≥ 5` over-cap → out of #136 scope**. The over-cap case (`n >= 5`) is the caller's own contract violation — the proxy adds 0 and never worsens it, but Anthropic still 400s (see operational-pitfalls #20).
